...
export class WhatsAppController {
  ...
  static async sendDynamic(req: Request, res: Response): Promise<void> {
    try {
      ...
      const components = TemplateBuilder.buildWhatsAppComponents(params, config || {
        name: templateName,
        requiredFields: [],
        optionalFields: [],
      ...
      const result = await WhatsAppService.sendTemplate(to, templateName, langCode, components);
      const statusCode = result.ok ? 200 : (result.error?.status || 500);
      res.status(statusCode).json(result);
    } catch (error) {
      const errorMessage = error instanceof Error ? error.message : 'Unknown error';
