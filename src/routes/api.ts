...
import {
  ...
  appendNotificationSettingLog,
  appendWidgetLog,
} from "../utils/logApiResponse.js";
const router: Router = express.Router();

// Apply authentication: JWT (Bearer), CUSTOM_TOKEN (Bearer), or x-api-key (API_SECRET)
router.use(authenticateJWTOrApiSecret);
// Apply authentication middleware to ALL routes
// router.use(authenticateToken); CUSTOM TOKEN
  ...
  async (_req: Request, res: Response): Promise<void> => {
    try {
      ...
      if (
        ...
        ErrorHandler.sendUnavailable(
          res,
          "Cannot connect to database. Please check your database credentials and ensure MySQL is running.",
        );
        return;
      }
